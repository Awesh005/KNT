...
exports.donationService = {
    async createDonation(data, userId) {
        if (!userId && (!data.donor_name || !data.donor_email)) {
            throw new errors_1.ValidationError('Name and email are required for guest donations');
        }
        if (data.screenshot_base64) {
            ...
            const uploadPath = path_1.default.join(__dirname, '../../../uploads/screenshots');
            if (!fs_1.default.existsSync(uploadPath)) {
                fs_1.default.mkdirSync(uploadPath, { recursive: true });
            }
            fs_1.default.writeFileSync(path_1.default.join(uploadPath, fileName), base64Data, 'base64');
            data.screenshot_url = `/uploads/screenshots/${fileName}`;
        }
        const donationId = 'DON-' + (0, crypto_1.randomUUID)().split('-')[0].toUpperCase();
        const campaignId = data.campaign_id === '' || data.campaign_id === undefined || data.campaign_id === null
            ? null
            : Number(data.campaign_id);
        await donation_model_1.donationModel.createDonation(donationId, {
            ...data,
            donor_id: userId,
        ...
        void mail_service_1.mailService.sendDonationReceived(donation);
        return donation;
    },
    async finalizeVerified(id, paymentRef) {
        const current = await this.getDonationById(id);
        if (current.status === 'verified')
            return current;
        const donation = await this.updateDonationStatus(id, 'verified', paymentRef);
        const { documentService } = await Promise.resolve().then(() => __importStar(require('../documents/document.service')));
        try {
    ...
    async getPublicStats() {
        return donation_model_1.donationModel.getPublicStats();
    },
    async getDonations(query, donorId) {
        ...
        const { donations: rawDonations, total } = await donation_model_1.donationModel.getDonations(filters);
    ...
    async getDonationById(id) {
        const donation = await donation_model_1.donationModel.getDonationById(id);
        if (!donation)
            throw new errors_1.NotFoundError('Donation not found');
        return donation;
    },
    async updateDonationStatus(id, status, paymentRef) {
        ...
        if (status === 'refunded') {
            ...
            throw new errors_1.AppError(`Cannot update status of a ${donation.status} donation`, 400);
        }
        ...
        try {
            ...
            if (status === 'verified' && donation.campaign_id) {
                await connection.query('UPDATE campaigns SET raised_amount = raised_amount + ? WHERE id = ?', [donation.amount, donation.campaign_id]);
            }
            if (status === 'refunded' && donation.campaign_id) {
                await connection.query('UPDATE campaigns SET raised_amount = GREATEST(raised_amount - ?, 0) WHERE id = ?', [donation.amount, donation.campaign_id]);
            }
    ...
    async deleteDonation(id) {
        ...
        try {
            ...
            if (donation.screenshot_url) {
                const screenshotPath = path_1.default.join(__dirname, '../../..', donation.screenshot_url);
                if (fs_1.default.existsSync(screenshotPath)) {
                    fs_1.default.unlinkSync(screenshotPath);
                }
            }
            if (donation.receipt_url) {
                const receiptPath = path_1.default.join(__dirname, '../../..', donation.receipt_url);
                if (fs_1.default.existsSync(receiptPath)) {
                    fs_1.default.unlinkSync(receiptPath);
                }
            }
            return true;
        }

...
export function UrgentCauses() {
  const { data, isLoading } = useSWR('/campaigns?is_urgent=true&limit=3', fetcher);
  ...
  return (
    <section className="py-24 bg-[#FCF8F7] border-y border-[#F0E6E4]">
        ...
        >
          <div>
            <motion.div variants={fadeInUp} className="flex items-center gap-2 mb-2">
              ...
              <Typography variant="overline" className="text-red-600 tracking-widest font-bold">Urgent Help Needed</Typography>
            </motion.div>
            <motion.div variants={fadeInUp}>
              <Typography variant="h2" className="text-charcoal">Save Lives Today</Typography>
            </motion.div>
          </div>
          <motion.div variants={fadeInUp}>
             <Link to="/campaigns?urgent=true">
               <Button variant="outline" className="border-red-600 text-red-600 hover:bg-red-50">View All Urgent Causes</Button>
             </Link>
          </motion.div>
        </motion.div>
        ...
        {isLoading ? (
          ...
          >
            {campaigns.slice(0, 3).map((campaign) => (
              <motion.div key={'urgent-'+campaign.id} variants={fadeInUp} className="h-full">
                <Card 
                  className="h-full flex flex-col group cursor-pointer hover:shadow-2xl transition-all duration-300 border-red-100"
                ...
                >
                  <div className="relative h-64 overflow-hidden">
                    ...
                    <div className="absolute top-4 left-4">
                      <span className="bg-red-600 text-white px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-[0.16em]">
                        Urgent
                      </span>
                    </div>
                  </div>
                  ...
                  <CardHeader className="border-b-0 pb-0 pt-6">
                    <Typography variant="h3" className="mb-2 line-clamp-2">{campaign.title}</Typography>
                  ...
                  <CardBody className="flex-grow">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="w-full bg-red-100 rounded-full h-2 overflow-hidden">
                        <div 
                          className="bg-red-600 h-full rounded-full transition-all duration-1000 relative"
                          style={{ width: `${Math.min(100, (campaign.raisedAmount / campaign.targetAmount) * 100)}%` }}
                        >
                          <div className="absolute inset-0 bg-white/20 w-full h-full animate-[marquee_2s_linear_infinite]" style={{ backgroundSize: '200% 100%', backgroundImage: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.4), transparent)' }} />
                        </div>
                      </div>
                    </div>
                  </CardBody>
                  ...
                  <CardFooter className="bg-transparent border-t border-charcoal/5 flex items-center justify-between py-4">
                    <div className="flex items-center gap-2 text-charcoal bg-red-50 px-3 py-1.5 rounded-full">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        <button 
                          onClick={(e) => { 
                            e.stopPropagation(); 
                            setOpenShareId(openShareId === 'urgent-' + campaign.id ? null : 'urgent-' + campaign.id);
                          }}
                        ...
                        {openShareId === 'urgent-' + campaign.id && (
                          <div 
                          ...
                          >
                            {/* WhatsApp */}
                            <a 
                              href={`https://wa.me/?text=Check out this urgent campaign: ${window.location.origin}/campaigns/${campaign.id}`}
                              target="_blank"
                      ...
                      <Button 
                        variant="primary" 
                        className="bg-red-600 border-red-700 hover:bg-red-700 hover:border-red-800"
                        onClick={() => navigate(`/campaigns/${campaign.id}`)}
                      >
                        Donate Now
                      </Button>
